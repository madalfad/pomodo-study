...
import { findPreset, type VideoPreset } from "@/lib/videoPresets";
...
interface YouTubeEmbedProps {
  defaultUrl: string;
  title: string;
  /** Optional list of preset videos shown in the settings dropdown. */
  presets?: VideoPreset[];
...
const YouTubeEmbed: FC<YouTubeEmbedProps> = ({
  ...
  useEffect(() => {
    ...
    const currentTimerType =
      localStorage.getItem("currentTimerType") || "focus";
  ...
  // Load YouTube API once (shared across every YouTubeEmbed instance) and
  // initialize this embed's player when both the API and a video ID are ready.
  useEffect(() => {
    if (!videoId) return;
    ...
    loadYouTubeIframeApi()
      ...
      .catch((err) => {
        console.error(`Failed to load YouTube API for ${title}:`, err);
      });
    ...
    return () => {
      ...
      if (playerRef.current) {
        try {
          ...
          console.error(`Error destroying ${title} player:`, err);
        }
  ...
  const initializePlayer = () => {
    ...
    try {
      ...
      console.log(
        `Creating player with ID: ${playerId} in container: ${containerId}`,
      );
      ...
      // Configure player. Use 100% for both dimensions so the iframe fills
      // its (16:9) container exactly instead of being a fixed 200px tall and
      // leaving black bars below.
      const playerConfig = {
        height: "100%",
        width: "100%",
        videoId: videoId,
        ...
        events: {
          onReady: (event: any) => {
            ...
            try {
              ...
              if (
                ...
                console.log(
                  `${title} player: Set initial volume to ${volume}%`,
                );
                ...
                if (typeof playerRef.current.getVolume === "function") {
                  try {
                    ...
                    console.log(
                      `${title} player: Verified volume is set to ${actualVolume}%`,
                    );

                    // If there's a mismatch, try setting it again
                    if (actualVolume !== volume) {
                      console.log(
                        `${title} player: Volume mismatch, resetting to ${volume}%`,
                      );
                      playerRef.current.setVolume(volume);
                    }
                  } catch (volError) {
                ...
                console.error(
                  `${title} player: Cannot set volume - missing setVolume method`,
                );
              }
              ...
              if (videoTitle && onTitleChange) {
                console.log(
                  `Retrieved video title for ${title}: ${videoTitle}`,
                );
              ...
              if (isLivestream) {
                ...
                if (duration && duration > 0) {
                  console.log(
                    `Seeking to end of livestream, duration: ${duration}`,
                  );
                  playerRef.current.seekTo(duration, true);
                } else {
                  console.log(
                    `Cannot determine livestream duration, continuing without seeking`,
                  );
                }
              }
            } catch (error) {
          ...
          onError: (event: any) => {
            ...
            console.error(
              `YouTube player error (${errorCode}): ${(errorCodes as any)[errorCode] || "Unknown error"}`,
            );
      ...
      // Create new player. `loadYouTubeIframeApi()` has already resolved by
      // the time we get here, so window.YT is guaranteed to exist.
  ...
  const smoothVolumeTransition = (targetVol: number) => {
    ...
    console.log(
      `Starting volume transition: ${transitionRef.current.startVolume}% → ${targetVol}%`,
    );
    ...
    const animateVolume = (timestamp: number) => {
      ...
      const currentVol = Math.round(
        transitionRef.current.startVolume +
          (transitionRef.current.targetVolume -
            transitionRef.current.startVolume) *
            progress,
      );
      ...
      if (
        ...
        try {
          ...
          if (progress === 0 || progress === 1 || progress % 0.25 < 0.01) {
            console.log(
              `${title} volume transition ${Math.round(progress * 100)}%: ${currentVol}%`,
            );
          }
        } catch (error) {
          ...
          if (iframe) {
            try {
              ...
              if (iframeWindow && iframeWindow.postMessage) {
                ...
                if (progress === 1) {
                  console.log(
                    `Completed volume transition via iframe API: ${currentVol}%`,
                  );
                }
              }
            } catch (e) {
  ...
  const handleVolumeChange = (newVolume: number) => {
    ...
    if (
      ...
      if (container) {
        ...
        if (iframe) {
          console.log(
            `Found iframe for ${title} player, attempting direct access`,
          );
          try {
            ...
            if (player && typeof player.setVolume === "function") {
              ...
              console.log(
                `Set ${title} volume via iframe direct access: ${newVolume}%`,
              );
            }
          } catch (e) {
  ...
  // Apply a preset's URL to the player. This effectively does the same thing
  // as the user pasting a URL and clicking "Update".
  const applyPresetUrl = (newUrl: string) => {
    setInputUrl(newUrl);

    // Tear down the current player and rebuild against the new video id.
    if (playerRef.current) {
      try {
        playerRef.current.destroy();
        playerRef.current = null;
        ...
        console.error(
          `Error destroying ${title} player on preset change:`,
          error,
        );
      }
    }
    ...
    // Clear-then-set forces a state change even if the URL happens to match.
    setUrl("");
    setTimeout(() => {
      setUrl(newUrl);
  ...
  // The value the <Select> shows. Either one of the preset URLs, or the
  // sentinel "custom" value when the user has pasted their own URL.
  const currentPresetValue =
    presets && findPreset(presets, url) ? url : CUSTOM_PRESET_VALUE;
  ...
  useEffect(() => {
    const handlePomodoroStateChange = (event: CustomEvent) => {
      ...
      if (timerType === "focus") {
        ...
        const currentVol =
          playerRef.current && typeof playerRef.current.setVolume === "function"
        ...
        console.log(
          `Transitioning to focus volume: ${initialVolume}% from ${transitionRef.current.startVolume}%`,
        );
        ...
        console.log(
          `Transitioning to break volume: ${breakVolume}% from ${transitionRef.current.startVolume}%`,
        );
  ...
  return (
    <div className="youtube-embed">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  if (value === CUSTOM_PRESET_VALUE) {
                    // Just reveal the URL input; don't change the current
                    // video. The user will paste their own URL below.
                    return;
                  }
                  applyPresetUrl(value);
                }}
              >
      ...
      <div className="relative rounded-md overflow-hidden bg-black mb-3 aspect-video">
        <div
          id={`youtube-container-${title.toLowerCase().replace(/\s/g, "-")}`}
          ...
          onValueChange={([newVolume]) => {
            ...
            if (
              ...
              try {
                playerRef.current.setVolume(newVolume);
                console.log(
                  `${title} slider: Set player volume to ${newVolume}%`,
                );
              } catch (error) {
                console.error(`Error setting ${title} player volume:`, error);
              }
              ...
              console.warn(
                `${title} player not available for direct volume change via slider`,
              );
              ...
              if (container) {
                ...
                if (iframe) {
                  console.log(
                    `Found iframe for ${title} player, attempting direct access from slider`,
                  );
                  try {
                    ...
                    if (iframeWindow && iframeWindow.postMessage) {
                      ...
                      iframeWindow.postMessage(message, "*");
                      console.log(
                        `Posted volume message to ${title} iframe: ${newVolume}%`,
                      );
                    }
                  } catch (e) {
                    console.error(
                      `Failed to access ${title} player via iframe from slider:`,
                      e,
                    );
                  }
                }
...
// The `window.YT` / `window.onYouTubeIframeAPIReady` typings live in
// `src/lib/youtubeApi.ts` so they're shared across every consumer of the API.
